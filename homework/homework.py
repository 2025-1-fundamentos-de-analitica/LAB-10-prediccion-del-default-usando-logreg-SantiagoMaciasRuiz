...
def load_data():
    base_dir = os.path.dirname(os.path.dirname(__file__))  # Sube un nivel desde /homework/
    input_path = os.path.join(base_dir, "files", "input")

    train_file = os.path.join(input_path, "train_data.csv.zip")
    test_file = os.path.join(input_path, "test_data.csv.zip")

    # Verificación opcional:
    if not os.path.exists(train_file):
        raise FileNotFoundError(f"No se encontró: {train_file}")
    if not os.path.exists(test_file):
        raise FileNotFoundError(f"No se encontró: {test_file}")
